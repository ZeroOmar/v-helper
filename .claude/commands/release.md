...
Otherwise, read the current version from `api.py` (`__version__ = "x.y.z"`) and decide the bump by reviewing the diff (step 2 below). Apply semver rules:
...
Edit `api.py`: change `__version__ = "OLD"` → `__version__ = "NEW"`.
...
Pushing the tag triggers `.github/workflows/release.yml`, which reads the annotated tag message and publishes it to the GitHub Releases page. Confirm the run succeeded (the release should appear at `https://github.com/ZeroOmar/v-helper/releases/tag/NEW_VERSION`).
